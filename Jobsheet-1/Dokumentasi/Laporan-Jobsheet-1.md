...
Kode di atas merupakan struktur dasar HTML yang dibagi mejadi 2 tag utama, yaitu `<head>` dan `<body>`. Tag `<head>` adalah tag yang berisi elemen HTML yang tidak ditampilkan secara langsung pada halaman web seperti `<title>`, `<script>`, dan `<style>`. Sedangkan tag `<body>` adalah tag yang berisi elemen HTML yang ditampilkan pada halaman web. Bayangkan tag `<body>` seperti kerangka dari halaman web yang mengatur tata letak dan elemen-elemen yang ditampilkan.
